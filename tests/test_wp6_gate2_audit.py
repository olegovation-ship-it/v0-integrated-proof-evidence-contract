...
def test_gate_registry_has_twenty_gates_and_one_pending():
 x=load('audit/WP6_GATE2_ACCEPTANCE_GATES.json');assert x['gate_count']==20;assert x['pass_count']==19;assert x['pending_count']==1;assert x['fail_count']==0;assert not x['gate2_closed']
...
def test_hosted_evidence_is_explicitly_pending_and_not_fabricated():
 x=load('evidence/WP6_HOSTED_CI_EVIDENCE_INTAKE_TEMPLATE.json');assert not hosted_pass(x);assert x['overall_status']=='PENDING' and x['evidence_source']=='NOT_RECORDED';assert x['run_id'] is x['run_url'] is x['head_sha'] is None;assert all(j['status']=='PENDING' and j['job_id_numeric'] is None for j in x['jobs'])
def test_audit_decision_holds_closure():
 x=load('audit/WP6_GATE2_AUDIT_RESULT.json');assert x['status']=='HOLD';assert x['decision']=='HOLD_FOR_HOSTED_CI';assert not x['gate2_closed'] and not x['closure_eligible'];assert x['gate_summary']=={'total':20,'pass':19,'pending':1,'fail':0}
def test_closure_record_creates_no_release_action():
 x=load('release/v0.1/WP6_GATE2_CLOSURE_RECORD.json');assert x['decision']=='HOLD_FOR_HOSTED_CI';assert not x['gate2_closed'] and not x['closure_authorized'];assert not any(x['release_actions'].values())
...
def test_wp6_manifest_hashes_match():
 x=load('release/v0.1/WP6_GATE2_AUDIT_MANIFEST.json');assert x['decision']=='HOLD_FOR_HOSTED_CI';assert all(sha(p)==h for p,h in x['files'].items())
...
def test_closure_eligibility_has_one_blocker():
 x=load('evidence/WP6_CLOSURE_ELIGIBILITY_REPORT.json');assert x['closure_eligible_after_hosted_ci'];assert x['remaining_blockers']==['G2-20_HOSTED_UPSTREAM_REPLAY_NOT_RECORDED']
