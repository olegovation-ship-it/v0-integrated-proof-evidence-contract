...
def objects():
 ...
 local={'artifact_id':'V0_IPEC_WP6_LOCAL_AUDIT_EVIDENCE','contract_id':'V0-IPEC-0.1','version':'0.1','date':DATE,'status':'PASS','metrics':m,'wp5_predecessor_sha256':load('baseline/WP5_BASELINE_REFERENCE.json')['wp5_package_sha256'],'local_checks_passed':19,'hosted_checks_executed':0,'frozen_upstreams_modified':False}
 eligibility={'artifact_id':'V0_IPEC_WP6_CLOSURE_ELIGIBILITY_REPORT','contract_id':'V0-IPEC-0.1','version':'0.1','date':DATE,'status':status,'closure_eligible_after_hosted_ci':fc==0 and pd==1,'remaining_blockers':['G2-20_HOSTED_UPSTREAM_REPLAY_NOT_RECORDED'] if pd else [],'automatic_reaudit_condition':'Replace the pending hosted-CI intake record with authentic GitHub Actions evidence and rerun the WP6 verifier.'}
...
def main():
 ...
 print(json.dumps({'status':'HOLD','gates':'19/20 PASS','pending':'G2-20 hosted CI evidence','gate2_closed':False},indent=2));return 0
